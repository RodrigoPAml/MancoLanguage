...
{
    /// <summary>
    /// Valida comando de break
    /// </summary>
    public class Break : SyntaxTree
    {
        public override void Validate(int position, List<Token> tokens, Stack<Scope> scopes)
        {
            // Continue nunca pode estar no escopo global
            if (scopes.First().Type == ScopeType.Global)
                throw new SyntaxException($"Invalid break in global scope", tokens[position - 1], ErrorCode.Break);

            // Continue precisa sempre estar dentro de um loop
            if (!scopes.Any(x => x.Type == ScopeType.Loop))
                throw new SyntaxException($"Invalid break outside of loop scope", tokens[position - 1], ErrorCode.Break);

            // Continue aparece sempre sozinho, caso contrario gera exceção
            if (position == tokens.Count())
                return;

            throw new SyntaxException($"Break must appear alone", tokens[position - 1], ErrorCode.Break);
        }
    }
}
